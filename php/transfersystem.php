<?php
	...
	if(!empty($assignid) and !empty($location) and !empty($toassignid))
	{
		...
		if($locationsql)
		{
			...
			while($i<=$toassignid)
			{
				...
				if(mysqli_num_rows($checksyssql)>0)
				{
					...
					{
					// 	$sql5 = mysqli_query($conn,"INSERT INTO log(user_id,id,description,purpose) VALUES({$_SESSION['unique_id']},'{$i}','Loaction transfered is - {$location}','Assign location to System.')");
					// 	$sql6 = mysqli_query($conn,"INSERT INTO log(user_id,id,description,purpose) VALUES({$_SESSION['unique_id']},'{$system['mouse_id']}','Loaction transfered is - {$location}','Assign location to System - mouse')");
					// 	$sql7 = mysqli_query($conn,"INSERT INTO log(user_id,id,description,purpose) VALUES({$_SESSION['unique_id']},'{$system['keyboard_id']}','Loaction transfered is - {$location}','Assign location to System - keyboard')");
					// 	$sql8 = mysqli_query($conn,"INSERT INTO log(user_id,id,description,purpose) VALUES({$_SESSION['unique_id']},'{$system['monitor_id']}','Loaction transfered is - {$location}','Assign location to System - monitor')");
					// 	$sql9 = mysqli_query($conn,"INSERT INTO log(user_id,id,description,purpose) VALUES({$_SESSION['unique_id']},'{$system['cpu_id']}','Loaction transfered is - {$location}','Assign location to System - CPU')");
